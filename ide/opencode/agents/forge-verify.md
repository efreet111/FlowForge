...
7. **Coverage Gate on git diff (NEW)**: run coverage on git diff files. Require ≥80% coverage on diff lines. REWORK if <80% with ≥5 lines. PASS_DEGRADADO if <80% with <5 lines. If tools unavailable → enforce mental mutation checklist.
