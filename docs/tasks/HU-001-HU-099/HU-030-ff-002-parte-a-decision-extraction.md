...
- [x] AC-1: Each `[DECISION]` row in plan.md is extracted and saved as an engram memory with type `decision`
- [x] AC-2: Each `[CONVENTION]` mention in plan.md is extracted and saved as an engram memory with type `convention`
- [x] AC-3: Capability matrix entries are extracted and saved as engram memories with type `capability`
- [x] AC-4: All captures are tagged with the plan's identifier (feature-slug) for traceability
- [x] AC-5: Capture is opt-in and handles failures gracefully without blocking plan finalization

## Scenarios (SDD Spec)
### Happy Path
- [x] **[Extract DECISION rows to engram]**
...
- [x] **[Extract CONVENTION mentions to engram]**
...
- [x] **[Extract capability matrix to engram]**
...
## Definition of Done
- [x] Plan agent or post-Plan hook extracts `[DECISION]` rows and saves to engram
- [x] Plan agent or post-Plan hook extracts `[CONVENTION]` mentions and saves to engram
- [x] Plan agent or post-Plan hook extracts capability matrix entries and saves to engram
- [x] All captures tagged with feature-slug for traceability
- [x] Capture failures are handled gracefully (logged, not blocking)
- [x] Tests verify extraction behavior for all three content types
- [x] Documentation updated to reflect decision capture flow
